...
export default function GraphView({ caseId = null, refreshTrigger = null }) {
  ...
  const stats = useMemo(() => {
    if (!nodes.length) return { total: 0, highRisk: 0, avgScore: 0, flagCount: 0 };
    ...
    nodes.forEach(n => {
      (n.flags || []).forEach(f => uniqueFlags.add(typeof f === "string" ? f : f.type || JSON.stringify(f)));
      (n.patterns || []).forEach(p => uniqueFlags.add(typeof p === "string" ? p : p.name || JSON.stringify(p)));
    });
  ...
  const handleDeepAnalysis = async (walletId) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/wallet/fraud-score?address=${walletId}`);
    ...
    const fetchGraphData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const simLinks = safeLinks.map(l => ({ source: l[0], target: l[1] }));

          const simulation = d3.forceSimulation(simNodes)
            .force("link", d3.forceLink(simLinks).id(d => d.id).distance(120).strength(0.2))
  ...
  const draw = useCallback(() => {
    ...
    nodes.forEach(n => {
      ...
      const hasFlags = (n.flags && n.flags.length > 0) || (n.patterns && n.patterns.length > 0);
  ...
  if (error) return <div style={{ padding: 24, color: "#ff5c67" }}>⚠️ {error}</div>;
  if (loading) return <div style={{ padding: 24, color: "#626c70" }}>Loading blockchain graph...</div>;
  if (nodes.length === 0) return <div style={{ padding: 24, color: "#626c70" }}>No network data found. Try analyzing a wallet first.</div>;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#ff5c67" }}>
            {stats.highRisk}
            <span style={{ fontSize: 12, color: "var(--muted)", marginLeft: 6, fontWeight: 500 }}>({Math.round((stats.highRisk / (stats.total || 1)) * 100)}%)</span>
          </div>
        </div>
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
          <span style={{ fontSize: 10, color: "var(--muted)", fontWeight: 700, letterSpacing: ".05em" }}>NETWORK RISK INDEX</span>
          <div style={{ fontSize: 22, fontWeight: 800, color: scoreToColor(stats.avgScore) }}>
            {stats.avgScore} <span style={{ fontSize: 12, color: "var(--muted)", fontWeight: 400 }}>/ 100</span>
          </div>
        </div>
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 12 }}>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: 20, overflowY: "auto", maxHeight: 540, display: "flex", flexDirection: "column", gap: 16 }}>
          {selected ? (
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {selected.flags.map((flag, idx) => {
                      ...
                      return (
                        <div key={idx} style={{ padding: "4px 8px", borderRadius: 4, background: color + "20", color: color, fontSize: 10, fontWeight: 600, border: `1px solid ${color}40` }}>
                          {flagStr.replace(/_/g, " ")}
                        </div>
                      );
                    })}
            ...
            <div style={{ color: "var(--dim)", fontSize: 13, textAlign: "center", paddingTop: 80, paddingBottom: 80 }}>
              ↖️ Click a wallet node<br /> to inspect ML analysis
            </div>
          )}
        </div>
      </div>

...
export default function WalletsView({ caseId, toast }) {
  ...
  const addWallet = async () => {
    ...
    const existing = wallets.find(
      (w) => w.addr?.toLowerCase() === addr.toLowerCase(),
    );
    if (existing) {
  ...
  const filtered = query.trim()
    ? wallets.filter((w) =>
        w.addr?.toLowerCase().includes(query.trim().toLowerCase()),
      )
    : wallets;
      ...
      >
        ...
        <div style={{ overflowY: "auto", flex: 1 }} className="cx-scroll">
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ padding: 20, fontSize: 12, color: "var(--dim)" }}>
              {query.trim()
                ? "No matching tracked wallet found. Click 'Track Wallet' to add it."
                : "No wallets found. Enter an address to track it."}
            </div>
          )}

          {filtered.map((w) => (
            <div
              className={`wv-row ${selected?.addr === w.addr ? "active" : ""}`}
              key={w.addr}
              onClick={() => setSelected(w)}
            >
              <span className="wv-addr">
                <AddrChip value={w.addr} size={11.5} />
              </span>
              ...
              <span
                className="cx-badge"
                style={{
                  background: `${riskColor(w.risk || "medium")}22`,
                  color: riskColor(w.risk || "medium"),
                  width: "fit-content",
                }}
              >
        ...
        {selected ? (
          <>
            <div style={{ marginBottom: 18 }}>
              <AddrChip value={selected.addr} size={14} />
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              <div
                style={{
                  ...
                  height: 70,
                  borderRadius: "50%",
                  background: `conic-gradient(${riskColor(selected.risk || "medium")} 0 ${selected.score || 50}%, #20282b ${selected.score || 50}% 100%)`,
                  display: "grid",
                  placeItems: "center",
              ...
              <div>
                <div
                  style={{
                    color: riskColor(selected.risk || "medium"),
                    fontWeight: 800,
                    fontSize: 13,
                ...
                >
                  {selected.chain || "ethereum"} · {selected.txCount || 0}{" "}
                  transactions
                </div>
              </div>
            </div>

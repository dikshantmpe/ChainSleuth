...
export default function CaseDetailView({ caseItem, onBack }) {
  ...
  useEffect(() => {
    const fetchCasePatterns = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_BASE_URL}/api/cases/${caseItem.id}/patterns`);
        if (res.ok) {
          ...
          setPatterns([
            { badge: "DETECTED", title: "Extreme Value Outlier", desc: "Transfers exceeding 500 ETH detected in case scope." },
            { badge: "DETECTED", title: "Rapid Pass-Through", desc: "Funds moved across intermediary wallets in under 24hrs." },
            { badge: "ANALYZING", title: "Sanctions Check", desc: "Cross-referencing counterparty addresses with OFAC database." }
          ]);
        }
      } catch (err) {
  ...
  return (
    <div className="cx-fade-up" style={{ padding: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "var(--muted)" }}>
            {caseItem.investigator} · Opened {caseItem.date_opened} · <strong style={{color: "var(--lime)"}}>{caseItem.wallets || 0}</strong> wallets tracked
          </div>
        </div>
        <span className="cx-badge" style={{ background: `${riskColor(caseItem.risk || 'medium')}22`, color: riskColor(caseItem.risk || 'medium'), fontSize: 10.5, padding: "6px 12px" }}>
          {caseItem.status}
        </span>
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        {tab === "overview" && (
          loading ? <div className="cd-loading">Analyzing case data...</div> : (
            <div className="cx-fade">
              ...
              <form className="cd-wallet-form" onSubmit={handleLinkWallet}>
                ...
                />
                <button type="submit" className="cx-btn cx-btn-primary" disabled={isLinking || !newWallet.trim()} style={{ padding: "10px 16px" }}>
                  {isLinking ? "Tracking..." : "Track Wallet"}
                </button>
              </form>

              <h3 style={{ fontSize: 14, color: "var(--dim)", margin: "0 0 12px" }}>Automated AI Insights</h3>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                {patterns.map((p, i) => (
                  <div key={i} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: 18 }}>
                    <span className="cx-badge" style={{ border: `1px solid ${p.badge === 'DETECTED' ? 'rgba(182,255,0,.2)' : 'rgba(255,255,255,0.1)'}`, color: p.badge === 'DETECTED' ? "var(--lime)" : "var(--dim)", fontSize: 9 }}>
                      {p.badge}
                    </span>
                    <h3 style={{ fontSize: 14, margin: "12px 0 6px", color: "#edf2f0" }}>{p.title}</h3>
        ...
        {tab === "wallets" && <div style={{ marginTop: 8 }}><WalletsView caseId={caseItem.id} /></div>}
        {tab === "transactions" && <div style={{ marginTop: 8 }}><TransactionsView caseId={caseItem.id} /></div>}
        {tab === "graph" && <div style={{ marginTop: 8 }}><GraphView caseId={caseItem.id} /></div>}
        {tab === "ai" && <div style={{ marginTop: 8 }}><AIView caseId={caseItem.id} /></div>}
      </div>
    </div>
  );
}
      ...
      try {
        ...
        if (res.ok) {
          ...
          const freshAlerts = (data.alerts || []).map((a, i) => ({
            ...
            desc: a.desc || "System detected anomalous routing matching algorithmic risk profiles."
          }));
          ...
          setFeed((prevFeed) => {
            ...
            const unique = combined.filter((v, i, a) => a.findIndex(t => (t.title === v.title && t.wallet === v.wallet)) === i);
  ...
  return (
    <div className="cx-fade" style={{ padding: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "var(--lime)" }}>
          <span style={{ width: 8, height: 8, borderRadius: 99, background: "var(--lime)", boxShadow: "0 0 0 4px rgba(182,255,0,.15)", animation: "pulse 2s infinite" }} />
          Live feed — connected to port 5001
        </div>
        <button 
          onClick={() => setIsPolling(!isPolling)}
          style={{ background: "transparent", border: "1px solid var(--line)", color: "var(--dim)", fontSize: 11, padding: "4px 10px", borderRadius: 6, cursor: "pointer" }}
        >
            ...
            <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span className="cx-badge" style={{ background: a.sev === "high" ? "rgba(255,92,103,.12)" : a.sev === "medium" ? "rgba(255,189,74,.12)" : "rgba(113,128,135,.15)", color: a.sev === "high" ? "var(--danger)" : a.sev === "medium" ? "var(--warning)" : "var(--dim)" }}>
                    {a.sev.toUpperCase()}
                  </span>
                ...
                <div style={{ fontFamily: "monospace", fontSize: 10.5, color: "var(--dim)", marginTop: 6 }}>{a.wallet}</div>
              </div>

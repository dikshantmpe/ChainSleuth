...
export default function ReportsView({ toast }) {
  ...
  const handleDownload = async (report) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/cases/${report.case}/export-data`);
      const data = await res.json();
  ...
  return (
    <div className="cx-fade-up" style={{ padding: 24 }}>
      <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <h2 style={{ margin: 0, fontSize: 18, color: "#fff" }}>Court Report Generation</h2>
      </div>
      <p style={{ margin: "0 0 24px 0", fontSize: 12, color: "var(--dim)" }}>
        Export immutable forensic summaries of tracked wallets and ML risk assessments for legal proceedings.
      </p>

      {error && (
        <div style={{ padding: 16, background: "rgba(255,92,103,.1)", color: "var(--danger)", border: "1px solid var(--danger)", borderRadius: 12, marginBottom: 16, fontSize: 12 }}>
          ⚠️ {error}
        </div>
      )}
            ...
            <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--dim)", marginTop: 4 }}>
                  Case {r.case} · Compiled {r.generated} · Prepared by {r.investigator}
                </div>
              </div>
              <button 

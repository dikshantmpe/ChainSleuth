    ...
    const fetchTransactions = async () => {
      try {
        ...
        const url = searchAddress
          ? `${API_BASE_URL}/api/transactions/search?address=${searchAddress}`
  ...
  return (
    <div className="cx-fade-up" style={{ padding: 24 }}>
      <style>{`
        ...
        .tx-search input { flex: 1; background: var(--card); border: 1px solid var(--line); border-radius: 10px; padding: 12px 14px; color: #fff; font-size: 13px; }
        .tx-search input::placeholder { color: var(--dim); }
        .tx-search button { background: var(--lime); color: #081000; border: 0; border-radius: 10px; padding: 12px 20px; font-weight: 700; cursor: pointer; font-size: 13px; }
        ...
        .tx-nav button:hover:not(:disabled) { border-color: var(--lime); }
        .tx-empty { padding: 40px; text-align: center; color: var(--dim); }
        .tx-error { padding: 16px 20px; background: rgba(255,92,103,.1); border: 1px solid var(--danger); border-radius: 12px; color: var(--danger); margin-bottom: 16px; font-size: 12px; }
      `}</style>
      ...
      <div className="tx-container">
        ...
        {/* Error Message */}
        {error && <div className="tx-error">⚠️ {error}</div>}
